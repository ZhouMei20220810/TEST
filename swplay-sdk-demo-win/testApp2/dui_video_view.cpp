...
bool DuiVideoView::InitMouseEvents(TouchEventListener* e) {
...
bool DuiVideoView::OnMouseDown(EventArgs* args) {
	...
	if (touchEvent != NULL) {
		touchEvent->OnTouchEvent(SW_ACTION_DOWN, 1, percentX, percentY, force[0]);
	}
	//ALOGI("OnMouseDown, mouseX:%d, mouseY:%d, percentX:%f, percentY:%f, x:%d, y:%d", mouseX, mouseY, percentX, percentY, x[0], y[0]);
	return true;
}
bool DuiVideoView::OnMouseMove(EventArgs* args) {
	...
	if(clicktype == SW_ACTION_DOWN){
		...
		if (touchEvent != NULL) {
			touchEvent->OnTouchEvent(SW_ACTION_MOVE, 1, percentX, percentY, force[0]);
		}
	}
	return true;
}
bool DuiVideoView::OnMouseUp(EventArgs* args) {
	...
	if (touchEvent != NULL) {
		touchEvent->OnTouchEvent(SW_ACTION_UP, 1, 0, 0, force[0]);
	}
	return true;
}
...
void DuiVideoView::onRenderer() {
	if (touchEvent) {
		...
		static volatile int64_t lastRendererTime = 0;
		int64_t now = get_time_now_ms();
		if (now - lastRendererTime >= 100) {
			lastRendererTime = now;
			Invalidate();
			//ALOGI("onRenderer");
		}
	}
