...
#define SW_LOG_TAG "VirtualTileBox"
...
void VirtualTileBox::Refresh()
{
	...
	int nElementCount = 0;// GetElementCount();
	...
	nElementCount = (int)itemList.size();
	if (m_nMaxItemCount > nItemCount) {//add
		if((int)itemList.size() > nItemCount){
			int n = m_nMaxItemCount - nItemCount;
			int i = 0;
			...
			for (it = itemList.begin(); it != itemList.end(); it++) {
				...
				Item *pControl = CreateElement(item->getPadcode());
				this->Add(pControl);
				item->setControl(pControl);
				item->changleTo(kSubWindow);
				if ((++i) >= n) {
					break;
				}
			}
	...
	else if (m_nMaxItemCount < nItemCount) {//remove
		int n = nItemCount - m_nMaxItemCount;
		int i = 0;
		std::list<std::shared_ptr<ControlItem> >::reverse_iterator it;
		for (it = itemList.rbegin(); it != itemList.rend(); it++) {
			std::shared_ptr<ControlItem>& item = *it;
			if (item->getControlType() == kMainWindow) {
				continue;
			}

			Control *pControl = item->getControl();
			if (pControl == NULL) {
				continue;
			}
			item->changleTo(kNoneWindow);
			item->setControl(NULL);
			this->Remove(pControl);
			if ((++i) >= n) {
				break;
			}
		}
...
Item* VirtualTileBox::CreateElement(const std::string& padcode)
{
	//if (m_pDataProvider)
	//	return m_pDataProvider->CreateElement();
	Item* item = new Item(m_MainForm, padcode);
	ui::GlobalManager::FillBoxWithCache(item, L"testApp2/item.xml", ui::MyCreateControlCallback);
	item->InitSubControls();
	return item;
}
