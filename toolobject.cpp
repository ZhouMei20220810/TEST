...
void ToolObject::HttpPostReplaceInstance(QMap<int, int> mapId)
{
        ...
        else
        {
            if (doc.isObject())
            {
                ...
                if (HTTP_SUCCESS_CODE == iCode)
                {
                    ...
                    emit HttpPostReplaceInstanceSignals(map);
                }
                else
                {
                    MessageTips* tips = new MessageTips(strMessage);
...
// 订单接口-我的支付订单
void ToolObject::HttpGetMyOrder(int iPage, int iPageSize)
{
    QString strUrl = HTTP_SERVER_DOMAIN_ADDRESS;
            ...
            if (doc.isObject())
            {
                ...
                if (HTTP_SUCCESS_CODE == iCode)
                {
                    if (obj["data"].isObject())
                    {
                        ...
                        QJsonArray records = data["records"].toArray();
                        QMap<int, S_ORDER_INFO> mapOrderInfo;
                        if (records.size() > 0)
                        {
                            int iRecordsSize = records.size();
                            QJsonObject recordObj;
...
//清空订单
void ToolObject::HttpEmptyOrder()
{
    QString strUrl = HTTP_SERVER_DOMAIN_ADDRESS;
